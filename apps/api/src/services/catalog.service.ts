...
export function createInstrumentContractSpec(db: Db, input: { instrumentFamily: string; multiplier: string; tickSize: string; effectiveDate: string }) {
  return db.insert(schema.instrumentContractSpecs).values({ instrumentFamily: input.instrumentFamily, multiplierSatangPerPoint: parseMoneyOrZero(input.multiplier), tickSizePoints: parsePriceOrZero(input.tickSize), effectiveDate: input.effectiveDate }).returning().get();
}
...
export function toInstrumentContractSpecDto(row: schema.InstrumentContractSpec) { return { id: row.id, instrumentFamily: row.instrumentFamily, multiplier: money(row.multiplierSatangPerPoint) ?? "0", tickSize: price(row.tickSizePoints) ?? "0", effectiveDate: row.effectiveDate, isActive: row.isActive }; }
