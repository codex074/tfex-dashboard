...
export function SettingsPage() {
  ...
  const members = useQuery({ queryKey: ["members"], queryFn: () => api.get<AuthUser[]>("/admin/users"), enabled: isAdmin });
  ...
  return <div className="space-y-10">
    ...
    {isAdmin ? <><h2 className="text-2xl font-bold text-ink-deep">{t("settings.adminArea")}</h2><div className="grid gap-6 lg:grid-cols-2">
      <Card tone="brokers" emoji="👥" title={t("settings.members")} subtitle={t("settings.membersHelp")}>
